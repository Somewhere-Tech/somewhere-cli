...
import { isNewer } from '../dist/lib/update-check.js';
...
test('isNewer — pre-release tags ignored on the base version', () => {
  assert.equal(isNewer('1.0.0-beta.1', '1.0.0'), false); // same base → not newer
...
test('isNewer — unparseable inputs are not "newer" (fail-safe)', () => {
  ...
  assert.equal(isNewer('1.0', '1.0.0'), false); // too few parts
